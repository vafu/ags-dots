...
// The actual widget for each single notification is in ags/modulcommons/widgets/notification.js
...
import Notification from "commons/widgets/notification.js"
import { ConfigToggle } from "commons/widgets/configwidgets.js"
    ...
    const notifCount = Label({
        attribute: {
            updateCount: self => {
                ...
                if (count > 0)
                    self.label = `${count} ${"notifications"}`
                else
                    self.label = ""
            },
        },
