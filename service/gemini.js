...
import { fileExists } from "commons/files"
...
if (!fileExists(`${GLib.get_user_config_dir()}/gemini_history.json`)) {
    Utils.execAsync(
        ["bash", "-c", `touch ${GLib.get_user_config_dir()}/gemini_history.json`],
    ).catch(print)
...
class GeminiService extends Service {
    ...
    saveHistory() {
        ...
        Utils.writeFile(JSON.stringify(this._messages.map(msg => {
            const m = { role: msg.role, parts: msg.parts }; return m
        })), HISTORY_PATH)
    }
    ...
    send(msg) {
        ...
        const body =
        {
            "contents": this._messages.map(msg => { const m = { role: msg.role, parts: msg.parts }; return m }),
        ...
        const message = new Soup.Message({
            method: "POST",
            uri: GLib.Uri.parse(
                replaceapidom(`https://generativelanguage.googleapis.com/v1/models/${this.modelName}:streamGenerateContent?key=${this._key}`), GLib.UriFlags.NONE),
        })
