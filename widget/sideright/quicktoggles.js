...
import { BluetoothIndicator, NetworkIndicator } from "commons/widgets/statusicons.js"
...
export const ModuleNightLight = async (props = {}) => {
    if (!exec("bash -c 'command -v gammastep'"))
        return null
    return Widget.Button({
        attribute: {
            enabled: false,
...
export const ModuleCloudflareWarp = async (props = {}) => {
    if (!exec("bash -c 'command -v warp-cli'"))
        return null
    return Widget.Button({
        attribute: {
            enabled: false,
        ...
        onClicked: self => {
            ...
            if (self.attribute.enabled)
                ...
                Utils.execAsync("warp-cli disconnect").catch(print)
        },
        child: Widget.Icon({
...
export const ModuleIdleInhibitor = (props = {}) => Widget.Button({ // TODO: Make this work
    ...
    onClicked: self => {
        ...
        self.toggleClassName("sidebar-button-active", self.attribute.enabled)
        if (self.attribute.enabled)
            Utils.execAsync(["bash", "-c", `pidof wayland-idle-inhibitor.py || ${App.configDir}/scripts/wayland-idle-inhibitor.py`]).catch(print)
        else
