...
import { decode, encode, equals, UnionToIntersection } from "./util"
import { i3_MSG_TYPE } from "./enum"
...
function readIntoBuffer(stream: Gio.InputStream, bufferSize: number) {
    return new Promise<Uint8Array>((resolve, reject) => {
        stream.read_bytes_async(
            ...
            null,
            (source, res) => {
                const b = source.read_bytes_finish(res).get_data()
                if (b === null)
                    reject(Error("null bytes read"))
                else
                    resolve(b)
            },
        )
...
function writeBuffer(stream: Gio.OutputStream, buf: Uint8Array) {
    return new Promise<number>(resolve => {
        stream.write_bytes_async(
            ...
            null,
            (source, res) => {
                resolve(source.write_bytes_finish(res))
            },
        )
    })
}
...
export async function Connect() {
    ...
    const conn = new Gio.SocketClient()
        .connect(new Gio.UnixSocketAddress({ path: SWAYSOCK }), null)

    const is = new Gio.DataInputStream({
    ...
    async function listen() {
        while (true) {
            try {
                const offset = i3_MAGIC.length
                const b = await readIntoBuffer(is, i3_HEADER)

                if (b.length < offset)
                    throw new Error(`read ${b.length} but was ${offset}`)

                if (b === null)
                    throw new Error("null data read from socket!")
                ...
                if (length > 0) {
                    const buf = await readIntoBuffer(is, length)

                    if (buf === null || buf.length < length)
                        throw new TypeError("Unexpected end of payload")
                ...
                if (isEvent) {
                    ...
                    if (!resolve)
                        throw new Error("Unexpected response from i3")
                    if (type !== expected)
    ...
    const i3 = {
                ...
                await writeBuffer(
                    ...
                    i3fmt(
                        ...
                        payload && typeof payload === "object"
                            ...
                            payload || ""))
                return queue.push([type, r])
            })) as UnionToIntersection<
                {
                    [k in keyof Messages]: (type: k, payload: Messages[k]["payload"]) =>
                        Promise<Messages[k]["reply"]>;
                }[keyof Messages]
            >,
        ...
        subscribe: (payload: Messages[i3_MSG_TYPE.SUBSCRIBE]["payload"]) =>
            i3.message(i3_MSG_TYPE.SUBSCRIBE, payload),
        ...
        getConfig: () =>
            i3.message(i3_MSG_TYPE.GET_CONFIG, null),
        sendTick: (payload: Messages[i3_MSG_TYPE.SEND_TICK]["payload"]) =>
            i3.message(i3_MSG_TYPE.SEND_TICK, payload),
        sync: (payload: Messages[i3_MSG_TYPE.SYNC]["payload"]) =>
            i3.message(i3_MSG_TYPE.SYNC, payload),
        getBindingState: (payload: Messages[i3_MSG_TYPE.SYNC]["payload"]) =>
            i3.message(i3_MSG_TYPE.SYNC, payload),
    }
